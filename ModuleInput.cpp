...
ModuleInput::ModuleInput() : Module(), mouse({ 0, 0 }), mouse_motion({ 0,0 }), mouseSensitivity(1.0f) {
	...
	while (SDL_PollEvent(&event) != 0) {
		...
		case SDL_WINDOWEVENT:
			...
			case SDL_WINDOWEVENT_RESIZED:
				...
				App->renderer->WindowResized(App->window->GetWidth(), App->window->GetHeight());
				break;
			}
			break;
...
void ModuleInput::SetMouseSensitivity(float newSens) {
	mouseSensitivity = newSens;
}

const float& ModuleInput::GetMouseSensitivity()const {
